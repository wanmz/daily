...
def main():
    return_data = []
    # decode解码成utf-8的
    filename = Path+"2020-12-8-资金流向-个股资金流.txt".decode('utf-8')
    ...
    conn = connect_db()
    cursor = conn.cursor()
    sql = "insert into 个股资金流 values (%s,%s, %s, %s, %s, %s, %s,%s, %s, %s, %s,%s, %s, %s, %s)"
        ...
        return_data.append([_res['f12'], _res['f14'], _res['f2']
                           ...
                           , str(_res['f184']) + '%', _res['f66'], str(_res['f69']) + '%', _res['f72']
                            , str(_res['f75']) + '%', _res['f78'], str(_res['f81']) + '%', _res['f84']
                               , str(_res['f87']) + '%', time.strftime('%Y-%m-%d-%A', time.localtime())])
