...
def main():
    return_data = []
    # decode解码成utf-8的
    filename = Path+"2020-12-8-资金流向-主力排名.txt".decode('utf-8')
    ...
    conn = connect_db()
    cursor = conn.cursor()
    sql = "insert into 主力排名 values (%s,%s, %s, %s, %s, %s, %s,%s, %s, %s, %s,%s, %s, %s, %s)"
    for _res in res["diff"]:
        ...
        return_data.append([_res['f12'], _res['f14'], _res['f2']
                            , str(_res['f184']) + '%', _res['f225']
                           , str(_res['f3']) + '%', str(_res['f165']) + '%', _res['f263']
                            , str(_res['f109']) + '%', str(_res['f175']) + '%', _res['f264']
                            , str(_res['f160']) + '%', _res['f265'], _res['f100']
